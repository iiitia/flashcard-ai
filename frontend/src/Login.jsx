      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 20, padding: "2rem", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
          ...
          <div style={{ display: "flex", background: "#020817", borderRadius: 10, padding: "0.25rem", marginBottom: "1.75rem" }}>
            ...
            <Link to="/register" style={{ flex: 1, padding: "0.55rem", background: "none", borderRadius: 8, color: "#475569", fontWeight: 600, fontSize: "0.85rem", textAlign: "center", textDecoration: "none" }}>
              Create Account
            </Link>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                  placeholder="Your password" style={{ ...inp, paddingRight: "3rem" }}
                  onFocus={(e) => (e.target.style.borderColor = "#3b82f6")} onBlur={(e) => (e.target.style.borderColor = "#1e293b")}
                  onKeyDown={(e) => e.key === "Enter" && handleSubmit()} />
            ...
            <button onClick={handleSubmit} disabled={loading}
              ...
              {loading
                ? <><span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #334155", borderTopColor: "#3b82f6", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /> Signing in...</>
                : "Sign In →"}
            </button>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: "1.25rem", fontSize: "0.75rem", color: "#334155" }}>
          Don't have an account? <Link to="/register" style={{ color: "#3b82f6", textDecoration: "none" }}>Create one free</Link>
        </div>
      </div>
